...
case class LaunchdConf(
  ...
  def write(dest: Path): Unit = PlistWriter.writePretty(xml, dest)

object LaunchdConf:
  val DEFAULT_PLIST_DIR = Paths.get("/Library/LaunchAgents")

  def executable(displayName: String) =
    s"/Applications/$displayName.app/Contents/MacOS/JavaAppLauncher"
