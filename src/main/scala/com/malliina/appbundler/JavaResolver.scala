...
trait JavaResolver:
  /** Converts `javaHome` to a jdk/Contents/Home path.
    *
    * Parameter `javaHome` may be any of the following: a) a script that returns the directory, like
    * /usr/libexec/java_home b) a jdk8 dir c) a jdk8/Contents/Home dir
    *
    * @param javaHome
  ...
  def resolveJavaDirectory(javaHome: Path): Path =
    if !Files.isDirectory(javaHome) && Files.isExecutable(javaHome) then
      Paths.get(Process(javaHome.toString).lazyLines.head)
    else
      val expectedName = "Home"
      if javaHome.getFileName.toString != expectedName then
        val maybeHome = javaHome / "Contents" / expectedName
        if Files.isDirectory(maybeHome) then maybeHome
        else javaHome
      else javaHome
